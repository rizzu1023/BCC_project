...
//            return $bestBowlingStrikeRate;
            ...
            $sortedBestBowlingStrikeRate = $bestBowlingStrikeRate->sortBy('bw_sr');
        ...
        if ($type == "bestEconomy") {
            ...
            $bestEconomy->map(function ($element) {
                $bw_economy = $this->calculate_economy($element->bw_runs,$element->bw_over,$element->bw_overball);
                $bw_eco = (float)number_format((float)$bw_economy, 2, '.', '');
                return $element->bw_economy = $bw_eco;
            });

            $bestEconomy = $bestEconomy->reject(function ($element) {
                return $element->bw_economy <= 0;
            });

            $sortedBestEconomy = $bestEconomy->sortBy('bw_economy');
            return StatsResource::collection($sortedBestEconomy);
        }
    }
}
